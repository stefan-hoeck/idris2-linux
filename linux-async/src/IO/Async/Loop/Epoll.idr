...
workST m c s p poller =
  fromPrim $ \w =>
    let MkIORes q w := newRef Queue.empty w
...
stop : ThreadPool -> IO ()
stop tp = do
  stop tp.poller -- TODO: Wakeup poller and await poller thread
...
mkThreadPool (S k) = do
  ...
  p  <- mkPoller -- TODO: Register eventfd for waking up poller
